...
public class PerformanceTest extends GenericTest {
	...
	private static final List<GPSHeuristic<CalcudokuState>> heuristics = Arrays.asList(new H1(), new H2(), new H8());
				...
				for (int index = 0; index < files_count; index++) {
					for (int repeat = 0; repeat < max_repeat; repeat++) {
						...
						TimedResults<GPSSolution<CalcudokuRule, CalcudokuState>> timedResult = Timer
								.time(() -> CalcudokuApplication.getSolution(board, strategy, heuristic));

						GPSSolution<CalcudokuRule, CalcudokuState> solution = timedResult.getLast().getValue();
