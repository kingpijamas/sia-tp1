...
public class PerformanceTest extends GenericTest {
	...
	public void performanceTest2() throws IOException {
		...
		String[] inputFileName = { ".̣/src/test/resources/4X4_easy.json", "./src/test/resources/4x4_medium.json",
		...
		for (SearchStrategy strategy : SearchStrategy.values()) {
			...
			for (Heuristic h : heuristics) {
				...
				for (int index = 0; index < files_count; index++) {
					...
					// CalcudokuApplication.getSolution(board,
					// strategy, h);
					//
	...
	private void recordData(BufferedWriter writer, long time, GPSSolution<CalcudokuRule, CalcudokuState> solution,
		...
		if (solution != null) {
			...
			String str = String.format("%s; %d; %d; %d", strategy.toString(), time, exposions, swaps);
