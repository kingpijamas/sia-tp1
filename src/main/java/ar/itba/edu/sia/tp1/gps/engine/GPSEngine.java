...
public abstract class GPSEngine<R extends GPSRule, S extends GPSState<R, S>> {
    ...
    public void solve() {
        ...
        if (solution.isSuccess()) {
            ...
            System.out.println("Expanded nodes: " + solutionProcess.getExplosionsCount());
    ...
    private Comparator<GPSNode<R, S>> getStateComparator(SearchStrategy strategy) {
        // TODO: concat the (informed) comparators to heuristic comparators!
        switch (strategy) {
        case BFS:
            return (s1, s2) -> Integer.compare(s1.getGValue(), s2.getGValue());
        case DFS:
            return (s1, s2) -> Integer.compare(s2.getGValue(), s1.getGValue());
        case IDDFS:
